...
var Monthly_medication_preventive_graph = require('../../models/daily_medicine_preventive');
var date1;
var date2;
let app =express();

  monthly_medication_preventive_graph = function(req , res){
    date1 = new Date()
    date1.setDate(date1.getDate() + 1);
    date1.setHours(0,0,0,0);
    date2 = new Date()
    date2.setDate(date1.getDate() - 26);
    ...
    Monthly_medication_preventive_graph.aggregate([{$match:{Users_id:req.params._id}},{"$unwind":"$Daily"},
    {$match:{$and:[{"Daily.Medication_Date":{$lte:date1}},
    {"Daily.Medication_Date":{$gte:date2}}]}},
    {$group:{_id:"$Daily.Medication_Date","Total":{$sum:"$Daily.ActualPuffs"}}},
    {$sort:{_id:-1}}
    ])
    .then((todo) => {
      //console.log(todo);
...
module.exports = monthly_medication_preventive_graph;
